...
import com.foxinmy.deimos.type.QuestionType;
import com.foxinmy.deimos.dao.WordCustomDao;
import com.foxinmy.deimos.dao.WordDao;
import com.foxinmy.deimos.model.Question;
import com.foxinmy.deimos.model.Word;
import com.foxinmy.deimos.service.WordService;
...
public class WordServiceImpl implements WordService{
	...
	@Autowired
	private WordCustomDao wordCustomDao;
	...
	public List<Question> buildQuestions(Word word, List<String> similarWords) {
		...
		if(similarWords != null && similarWords.size() > 4) {
			for(int i=4; i<similarWords.size(); i++) {
				...
				Word similarWordBean = wordCustomDao.getBySpell(similarWord);
		...
		if(!word.getPhrase()) {
			...
			q1.setWord(word);
			q1.setType(QuestionType.TYPE_1.value);
			q1.setOrdinal(0);
			setOptionsAndRightIndex(q1, QuestionType.TYPE_1, new ArrayList<Word>(optionWords));
			questions.add(q1);
			
			Question q2 = new Question();
			q2.setWord(word);
			q2.setType(QuestionType.TYPE_2.value);
			q2.setOrdinal(1);
			setOptionsAndRightIndex(q2, QuestionType.TYPE_2, new ArrayList<Word>(optionWords));
			questions.add(q2);
			
			Question q3 = new Question();
			q3.setWord(word);
			q3.setType(QuestionType.TYPE_3.value);
			q3.setOrdinal(2);
			setOptionsAndRightIndex(q3, QuestionType.TYPE_3, new ArrayList<Word>(optionWords));
			questions.add(q3);
			
			Question q4 = new Question();
			q4.setWord(word);
			q4.setType(QuestionType.TYPE_4.value);
			q4.setOrdinal(3);
			questions.add(q4);
		} else {
			//短语
	 ...
	 * @param similarWords
	 */
	private void setOptionsAndRightIndex(Question q, QuestionType type,
